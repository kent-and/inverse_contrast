...
\caption{Show the  function defined in \eqref{scaledmagnetization} with the white region indicating $T_1$ values for white matter,the grey region indicating $T_1$ values for grey matter,the blue region indicating $T_1$ values for CSF.  }
\label{figuredti} 
\end{figure}
...
In Tabel~\ref{Tab::Noise0.03}, we can see that a noise amplitude of 0.03, (10$\%$ of maximum initial condition) had negligible effect on the relative error. Then in Tabel~\ref{Tab::Noise0.3} the noise amplitude was increased to 0.3 (100$\%$ of maximum initial condition), which gave a visible effect. The noise addition caused the number of iteration to increase, and in combination with more timesteps caused the optimization not to convergence for $\beta < 1.0$. Furthermore, the relative boundary error $||g||\sp{rel}_{L\sp{2}(\Omega_1)}$ is significantly larger for $\alpha< 1.0e-2 $ and $\beta < 1.0e-2$.  
