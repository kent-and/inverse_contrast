...
The code used the FEniCS project to solve the PDE, using backwards Euler time discretization and first order continuous Galerkin finite elements. The linear problem was solved using GMRES, and bilinear matrix stored in the solver. {\color{red} why not amg}  
The module dolfin adjoint ~\cite{farrell2013automated} \kam{correct ref?} was used to solve the PDE constrained optimization problem with the L-BFGS-B algorithm in the python module scipy. The convergence criteria for the minimization was set so that $L\sp{inf}$ norm of the project gradient was less than $gtol =1.0e-1$.
...
Each observations $u_{obs}$ have a fixed time-point $t_i$, these time-points may not overlap with the time discretization in the forward problem. Therefore, after each time step $dt$ in Eq\ref{Eq::PDE}, it was checked if the time step contained the next observation. If it did, then a linear interpolation of the current solution $U$ and previous solution $U(t-dt)$ was used approximate the solution corresponding to $t_i$. The linear interpolation was given as  
\begin{equation}
U_{lin}(t) = \frac{\Delta t}{dt} U(t-dt) + \frac{dt - \Delta t }{dt} U(t), \quad t \in \lbrace t-dt, t \rbrace
\end{equation}
with $\Delta t = t-t_i $
...
The initial values of the optimization algorithm was chosen to be $1,1,1$. Furthermore, to obtain faster convergence in the optimization, the diffusion coefficients was scaled to have the same magnitude, i.e. $D_{\Omega_1} = D\sp {s}_{\Omega_1}100$.  
...
\subsection{DTI}
The validation of the results from the MRI data can be done using diffusion tensor imaging (DTI). Diffusion tensor imaging is a MRI-sequence that is used to obtain the apparent diffusion coefficients of water in the brain. {\color{red} mer on DTI? } 

The diffusivity of the Gadovist (600 Da) is currently not known {\colro{red} Geir vet dere?}, thus the diffusion of (Navn Gd-DPTA (550 Da)  will be used as reference. The free diffusion coefficient for Gd-DPTA  3.8e-5$\mathrm{mm\sp{2}/s}$ , and obtained from [cite]. {\color{red} discussion om hvorfor eller nevne Stoke Einstein her?}
...
\label{Fig::Mesh}
\end{figure}
The 3 domain mesh in Fig.\ref{Fig::Mesh} A, consists of 244318 tetrahedral cells and 22057 vertices, while the 2 domain mesh Fig.\ref{Fig::Mesh} B consists of 335589 tetrahedral cells and 73002 vertices.
...
The robustness of the objective functional defined in Eq.\ref{Eq::F} is dependent on the regularization parameters $\alpha$ and $\beta$. Therefore it is important to select regularization parameter values that ensures an effective and accurate convergence. The evaluation of different regularization parameters was done by solving the optimization with different values of $\alpha$ and $\beta$ and compare the controls with the manufactured solution. The observation times were selected with $t_i \in  \lbrace$ 2.4, 4.8, 7.2, 9.6, 12.0, 14.4, 16.8, 19.2, 21.6, 24.0 $\rbrace$, and the number of time-steps in the forward problem was set to $k= 10$. 

The results are shown in Tab.\ref{Tab::1} together with the number of iteration used to reach the convergence criteria. For $\alpha= 1.0$, it can be observed that there is a significant relative error for $D_{\Omega_1}$. The other control parameters also show an increased relative error when compared to other values of $\alpha$. 
...
The PDE uses backward Euler, which is a linear approximation in time. This means that the increasing the number of timesteps will allow for a better temporal approximation. However, this will also increase the computational cost for each iteration of the L-BFGS-B algorithm. Thus finding the number of timesteps for an effective convergence can help for larger meshes. The computation vary the timesteps with 10, 20 and 40 timesteps. Additionally, the computation will use a few variations of the regularization parameters that gives small relative errors. 
...
The estimation of tracer concentration proved difficult in the CSF compartment. Therefore the 2 domain mesh, shown in Fig.\ref{figmesh} was used in the computation. Furthermore, bounds were added to the L-BFGS-B algorithm to ensure non-negative boundary controls and the convergence criteria was adjusted to $gtol=6.0e-1$. The resulting states can be seen in Fig. ?? and the relative error of control values are displayed in Tab.?? . 


\subsection*{DTI comparison}
The median diffusion coefficient in the DTI was estimated to be respectively in white and grey matter to be 8.7e-4 $\mathrm{mm\sp{2}/s}$ and  1.0e-3 $\mathrm{mm\sp{2}/s}$. This is an increase from the reported values in [insert citation] of XX and XX.  
...
The computational model assumes isotropic diffusivity, but the anisotropy in the white matter is well documented, as shown in Fig\ref{FIG::DTI}. 

The anisotropy can be seen by comparing the observations and the states after 48 hours, see Fig ??. This observation is after 48 hours, and the amount of tracer inside the anisotropic region is negligible. Therefore it would seem that the anisotropy do not have direct impact on the computations, since the computation can asses a static environment. 
  
A more important factor is that of tissue damage, which may causes higher diffusivity.  In Fig.\ref{FIG::DTI}, it can be observed that the region with high diffusivity also have a high concentration tracer, see Fig ??. Since resulting states corresponds with the observation indicates that the computed values are higher than the average ADC value.


\subsection{Real Data}

The computed diffusion coefficients are in the same magnitude as the values obtained from the DTI. But it can be seen that there is a clear difference between grey matter and white matter values.  


The grey matter diffusion coefficient displays a high volatility in the variation of the relaxation parameters, which can be seen in Tab.\ref{Tab::Real-data}. This volatility can be caused by the boundary control $g$, which exists along the entire boundary of the grey matter. Considering the that the grey matter volume has a depth less than 5 mm makes it susceptible to changes in $g$. Therefore this seems likely, given the fact that both relaxation parameters are used to adjust the boundary control. 
  



 boundary control, which exists along the external boundary of the grey matter. This  


A possible solution is to include the CSF compartment, but only use the observation in the white and grey matter. 

The computational have two values for diffusion coefficients, while it can be seen in  Fig.\ref{FIG::DTI} that diffusion coefficients can be considered a spacial function. 
The controls for diffusion coefficients should be spacial functions, but this would increase the computational cost greatly. 
...
The concentration estimation is a stepwise process, which required adjustment of the intensity for each observation \cite{eidevalnes}. Thus the accumulation of errors can be a factor in the estimation. 





The results of the computation using MRI data show a higher volatility of the regularization parameters. 


\section{Conclusion}
The results from the manufactured  


However, this method has limitations based on the model and on the acquired data. 

The adaptation for a 
...
\newgeometry{top=0.5cm}
 ...
 \begin{table}
\centering
\caption{ {\color{red} Preliminary data, new runs with different convergence criteria is running. No excepted change is assumed see Fig.\ref{fig:gull} } Shows the relaxation parameters $\alpha$ and $\beta$, number of timesteps $k$, the resulting number of iterations, the estimated diffusion coefficients based on MRI data.}
\resizebox{\textwidth}{!}{\begin{tabular}{*{6}c}
$\alpha$ & $\beta$ & k & iter &  $ ||D_{\Omega_1}||_{RE}$ & $ ||D_{\Omega_2}||_{RE}$ \\
\hline
 1.0e-02 	 & 1.0e+00 	 & 24 & 1001 	 & +0.658 & +0.914 \\ 
 1.0e-02 	 & 1.0e-02 	 & 24 & 1001 	 & +0.840 & +0.976 \\ 
 1.0e-06 	 & 1.0e+00 	 & 24 & 1001 	 & +0.572 & +0.832 \\ 
 1.0e-06 	 & 1.0e-02 	 & 24 & 1001 	 & +0.719 & +0.727 \\ 
...
\begin{figure}
  \begin{subfigure}[b]{0.5\textwidth}
        \includegraphics[width=\textwidth]{Pic_Grey.png}
        \caption{ {\color{red} shows that it converges but sufficent to the convergence criteria}}
        \label{fig:gull}
    \end{subfigure}
     \qquad
     \begin{subfigure}[b]{0.5\textwidth}
        \includegraphics[width=\textwidth]{Pic_White.png}
        \caption{ {\color{red} shows that it converges but sufficent to the convergence criteria} }
        \label{fig:gull}
    \end{subfigure}
\end{figure}
