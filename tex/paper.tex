...
In 2011,~\cite{iliff2012paravascular} discovered a novel system within our brain's metabolism system, which changed our understanding of the brain. 
...
is linked to the development of dementia such as Alzheimer's and Parkinson's diseases. In 2013,~\cite{xie2013sleep} found
...
the complex geometry of the brain is taken into account and time-scales of hours or days are taken into approach. 
The approach we will take is to solve a PDE constrained optimization problem 
via adjoint methods where we have sparse observations on selected time-points, around 10 acquisitions during 24 hours, but throught
...
the sparse observations throught the time domain.  As such we need to assess the sensitivity of
...
to obtain parameters involved time-scales beyond those of the MRI aquisition. We remark that the purpose
of this paper is a systematic study of the mathematical challenges faces and that assessing  
whether clearance is goverened by a process that is faster than diffusion is not the topic of the current paper.       
...
in FEniCS. In Section 3 we present the results and have a rather extensive discussion of the verification performed
...
\end{equation}  
The magnitude order for $\quad D_{\Omega_2} $ and $\quad D_{\Omega_2}$ were chosen to resemble diffusion coefficient for water in grey and white matter, but the relation between the coefficients were not preserved. The diffusion coefficient in the CSF was chosen given [cite Bryne and Kent], and the initial condition was set to 1.0 in $\Omega_1$ and 0 elsewhere. The manufactured forward computation gave a total of 120 possible observation time points. These points will be denoted as $t_i$.
...
Then by using the reference value $3.8e-4 \mathrm{mm\sp{2}/s}$ for Gd-DPTA gives an estimate for the diffusion coefficient in the grey and white matter to respectively be $ 1.26\mathrm{mm\sp{2}/s}$ and $1.10 \mathrm{mm\sp{2}/s}$. This estimation assumes that the tortuosity is independent on molecular size . 
...
The computation require that there is tracer present to compute the diffusion coefficient. In Fig.\ref{Fig::realdata}, it can be seen that there are regions with negligible amount of tracers and vice versa. Considering $1.3 \mathrm{mm\sp{2}/s}$ as the upper bound gives an estimated upper bound $1.43$ diffusion coefficient in the white matter. This relative error of $54\%$ difference {\color{red} compare with dispersion, NB bare k 48 er brukt, k 24 skal med}  
...
in the Figures \ref{12hourswithnoise} and \ref{24hourswithnoise} with noise levels of 30\% \kam{is this correctly stated}. 
