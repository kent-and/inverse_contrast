...
T
he noise susceptibility was also tested by adding an uniform distribution noise term to the observations. The noise term was constructed using numpy random and multiplied with a noise amplitude. The noise amplitudes were chosen to be $ 10\%, 100\%$ of the maximum initial boundary conditions, so that negative concentrations are avoided.  

The real data observations had observation times that had an unevenly distribution over 24 hours. Thus having constant time between observations is not realistic.
...
\subsection{DTI}
The validation of the results from the MRI data can be done using diffusion tensor imaging (DTI). Diffusion tensor imaging is a MRI-sequence can be used to obtain the apparent diffusion coefficients of water in the brain. {\color{red} mer on DTI? } 

The Stokes-Einstein relation combined with th approximation between weight and radius of particles
\begin{equation}
D = \frac{k_B T}{ 6 \pi n r} \qquad
M_w  = \frac{4}{3} \pi \rho r\sp{3}
\end{equation}
can be used to estimate the diffusion coefficient for Gadovist (600 Da). {\color{red} Trenger å verifiseres}
...
\end{equation}
with $X$ as an arbitrary scalar parameter. For the boundary parameter $g$, the relative error norm is defined as 
\begin{equation}
|| g ||_{RE} = \frac{||g_{found} -g_{true}||_{L(\Omega_1)} }{ ||g_{true}||_{L(\Omega_1)} }
\end{equation}


\subsection{The relaxation parameters}
The robustness of the objective functional defined in Eq.\ref{Eq::F} is dependent on the regularization parameters $\alpha$ and $\beta$. Therefore it is important to select regularization parameter values to ensure effective and accurate convergence. The evaluation of different regularization parameters was done by solving the optimization with different values of $\alpha$ and $\beta$ and compare the controls with the manufactured solution. The observation times were selected with $t_i \in  \rbrace$ 2.4, 4.8, 7.2, 9.6, 12.0, 14.4, 16.8, 19.2, 21.6, 24.0 $\lbrace$, and the number of time-steps in the forward problem was set to $k= 10$. 
...
The MRI data contains noise, hence an investigation to the noise susceptibility is needed. This is done by adding uniform noise in the range $\lbrace -n_{amp} , n_{amp} \rbrace $, with $n_{amp}$ defined as noise amplitude, to the manufactured observations. Then the optimization was solved by varying the regularization parameters $\alpha$ and $\beta$. 
...
The estimation of tracer concentration proved difficult in the CSF compartment. Therefore the 2 domain mesh, shown in Fig.\ref{figmesh} was used in the computation. Furthermore, bounds were added to the L-BFGS-B algorithm to ensure non-negative boundary controls. The resulting states can be seen in Fig. ?? and the relative error of control values are displayed in Tab.?? . 


\subsection*{DTI comparison}
The median diffusion coefficient in the DTI was estimated to be  respectively in white and grey matter to be 8.7e-4 $\mathrm{mm\sp{2}/s}$ and  1.0e-3 $\mathrm{mm\sp{2}/s}$. This gives a related diffusion coefficient of tracer to be ?? and  ??.
...
The computational model does not take into account the anisotropy in the brain, see Fig\ref{FIG::DTI}. 

The region with great FA In Fig\ref{FIG::DTI},  does not show mush concentration of csf. Thus the anisotropy does not seem to be a significant factor in the computation. This is due to the fact that the optimization require 

\subsection{Real Data}

The comparison with DTI use Stokes-Einstein to estimate the measured diffusion coefficient, this will give some error. Furthermore, comparison with molecules of size 600 Da shows  


The limited number observations makes it difficult to measure the grey matter diffusion coefficient, since domain depth is about 5 mm {\color{red} cortex thickness}.


Furthermore, the grey domain is more affected by the boundary control. This means that the 
diffusion paramet




The computational have 2 diffusion coefficient controls, while the DTI values represent a range of measurements, 



\section{Sleep}
There have been reports of increase clearance in rodents that are sleeping. This effect can only by a change in the tortuosity since the properties of the molecules do not change. This indicates that the model should take into account changes in the apparent diffusion coefficient. Thus making the control parameters time dependent.
