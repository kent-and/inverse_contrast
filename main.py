...
def iter_cb(m):
    ...
    print("DirichletBC-Iter: {} | {}".format(iter_cnt, sum([assemble((fenics_m[i] - correct_g[i-2])**2*ds(1)) for i in range(3, len(fenics_m))])/sum([assemble(correct_g[i-2]**2*ds(1)) for i in range(3, len(fenics_m)) ]) ))
